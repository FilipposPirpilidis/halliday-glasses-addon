...
The add-on only defines the allowed translation pairs. The active source/target pair must be set by the client at runtime through `translate-set`. If the client does not set a pair, the add-on stays in transcription-only mode.
